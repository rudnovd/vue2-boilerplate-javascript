...
export function checkRouteAuth(to, from, next) {
  const authenticationRequired = to.matched.some(
    record => record.meta.authenticationRequired
  );
...
export function checkRouteAdminRole(to, from, next) {
  const adminRoleRequired = to.matched.some(
    record => record.meta.adminRoleRequired
  );
...
export function setRouteProps(to) {
